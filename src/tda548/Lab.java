...
public class Lab implements Labyrinth {
    ...
    public Lab clone() {
        ...
        n.width = this.width;
        n.wallRight = new boolean[width][height];
        n.wallBelow = new boolean[width][height];
        n.annotations = new int[width][height];
        for (int x=0;x<width;x++) {
            for (int y=0;y<height;y++) {
                n.wallRight[x][y] = this.wallRight[x][y];
                n.wallBelow[x][y] = this.wallBelow[x][y];
                n.annotations[x][y] = this.annotations[x][y];
            }
        }
        return n;
    }
